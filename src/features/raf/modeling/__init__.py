...
def extract_traditional_features(images: np.ndarray) -> np.ndarray:
    ...
    for i, img in enumerate(images):
        try:
            ...
            kurtosis_val = (
                np.mean(((gray - mean_val) / std_val) ** 4) if std_val > 0 else 0
            )
            ...
            features_list.append(
                [0] * 70
            )  # 64 (hist) + 4 (moments) + 2 (énergie/entropie) + 2 (gradients)
...
def get_model_type(model_name: str) -> str:
    ...
    if any(
        ...
        for keyword in ["forest", "tree", "xgb", "lgb", "catb", "boost"]
    ):
        return "ensemble"
    elif any(
        keyword in model_name_lower
        for keyword in ["linear", "logistic", "ridge", "lasso"]
    ):
        return "linear"
        ...
        keyword in model_name_lower
        for keyword in ["neural", "cnn", "deep", "resnet", "vgg"]
    ):
        return "deep"
