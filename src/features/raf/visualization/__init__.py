...
def visualize_sample_images(
    image_paths: List[str], labels: List[str], classes: List[str], n_samples: int = 3
):
    """
...
def analyze_image_properties(image_paths: List[str], sample_size: int = 100):
    ...
    """
    print(
        f"📊 Analyse des propriétés sur {min(sample_size, len(image_paths))} images..."
    )
    ...
    print(
        f"   Min/Max: {np.min(brightness_values):.1f} / {np.max(brightness_values):.1f}"
    )
