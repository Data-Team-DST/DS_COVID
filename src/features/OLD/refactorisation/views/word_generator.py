...
def create_report_folder(folder_name="reports"):
    """Crée un dossier 'reports' s'il n'existe pas."""
    if not os.path.exists(folder_name):
        os.makedirs(folder_name)
    return folder_name


def generate_word_report(models_results, output_folder="reports", generate_pdf=True):
    """
    ...
    docx_path = os.path.join(report_folder, f"reports_classification_{timestamp}.docx")
    pdf_path = os.path.join(report_folder, f"reports_classification_{timestamp}.pdf")

    # Créer le document Word
    doc = Document()
    doc.add_heading('reports de Classification - Modèles ML', 0)
    ...
    for result in models_results:
        ...
        # reports de classification
        doc.add_heading("reports de classification", level=2)
