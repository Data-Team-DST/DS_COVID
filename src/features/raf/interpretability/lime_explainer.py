...
class LIMEExplainer:
    ...
    def __init__(self, model, preprocess_fn=None):
        ...
        if not LIME_AVAILABLE:
            raise ImportError(
                "LIME n'est pas disponible. Installer avec: pip install lime"
            )
    ...
    def explain_image(
        ...
        if segmentation_fn is None:
            segmentation_fn = SegmentationAlgorithm(
                "quickshift", kernel_size=4, max_dist=200, ratio=0.2, random_seed=42
            )
    ...
    def plot_lime_explanation(
        ...
        axes[0].imshow(
            ...
            cmap="gray" if original_image.shape[-1] == 1 else None,
        )
    ...
    def analyze_lime_features(
        self, results: Dict[str, Any], class_idx: Optional[int] = None
    ) -> Dict[str, Any]:
        """
    ...
    def compare_lime_explanations(
        ...
        for i, image in enumerate(images):
            try:
                ...
                print(
                    f"✅ {label}: {analysis['positive_features']} features positives, "
...
def visualize_lime_comparison(
    comparison_results: Dict[str, Any], class_names: Optional[List[str]] = None
) -> plt.Figure:
    """
    ...
    valid_explanations = [
        (exp, label) for exp, label in zip(explanations, labels) if exp is not None
    ]
    ...
    for i, (explanation, label) in enumerate(valid_explanations):
        ...
        # Image originale
        axes[0, i].imshow(
            explanation["original_image"].squeeze(),
            cmap="gray" if explanation["original_image"].shape[-1] == 1 else None,
        )
        class_name = (
            class_names[predicted_class] if class_names else f"Classe {predicted_class}"
        )
