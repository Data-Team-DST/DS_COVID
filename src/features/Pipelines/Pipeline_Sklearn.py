...
class MedicalImagePreprocessor(BaseEstimator, TransformerMixin):
    ...
    def __init__(
        self, enhance_contrast=True, noise_reduction=True, histogram_equalization=True
    ):
        """
        Args:
    ...
    def transform(self, X):
        ...
        if self.histogram_equalization:
            ...
            X_transformed = (X_transformed - X_transformed.min()) / (
                X_transformed.max() - X_transformed.min()
            )
...
class PipelineManager:
    ...
    def create_pipeline(self, config_name):
        ...
        for step in pipeline_config["steps"]:
            transformer = self._get_transformer(step["transformer"], step["params"])
    ...
    def train_pipeline(self, config_name, X_train, y_train, X_test=None, y_test=None):
        ...
        cv_scores = cross_val_score(
            base_pipeline, X_train, y_train, cv=cv_folds, scoring="accuracy"
        )
    ...
    def _save_pipeline(self, config_name, pipeline):
        ...
        output_dir = self.config["model_saving"].get(
        ...
        naming = self.config["model_saving"].get(
