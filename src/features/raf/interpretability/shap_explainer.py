...
class SHAPExplainer:
    ...
    def _detect_model_type(self, model) -> str:
        ...
        if any(
            ...
            for tree_type in ["tree", "forest", "gradient", "xgb", "lgb"]
        ):
            return "tree"
        elif any(
            linear_type in model_name for linear_type in ["linear", "logistic", "svm"]
        ):
            return "linear"
    ...
    def fit_explainer(self, X_background: np.ndarray, **kwargs):
        ...
        if self.model_type == "tree":
            ...
            self.explainer = shap.KernelExplainer(
                self.model.predict, X_background, **kwargs
            )
    ...
    def explain(self, X: np.ndarray, max_evals: int = 100) -> np.ndarray:
        ...
        if self.explainer is None:
            raise ValueError(
                "L'explainer doit être ajusté avec fit_explainer() d'abord"
            )
    ...
    def plot_waterfall(
        ...
        try:
            ...
            shap.waterfall_plot(
                expected_val, shap_vals, feature_names=feature_names, show=show
            )
    ...
    def plot_force(
        ...
        if matplotlib:
            ...
            return shap.force_plot(expected_val, shap_vals, feature_names=feature_names)
...
def explain_prediction(
    ...
    return {
        ...
        "waterfall_fig": explainer.plot_waterfall(
