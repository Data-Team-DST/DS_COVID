...
st.set_page_config(
    page_title="Interprétabilité - SHAP & GradCAM", page_icon="🔍", layout="wide"
)
...
with st.sidebar:
    ...
    shap_method = st.selectbox(
        "Méthode SHAP", ["auto", "tree", "linear", "deep", "kernel"]
    )
    ...
    class_names = st.text_area(
        "Noms des classes (un par ligne)", value="\n".join(default_classes)
    ).split("\n")

# Interface principale
tab1, tab2, tab3, tab4 = st.tabs(
    ["🖼️ Analyse Simple", "📊 Comparaison", "📈 Rapport Batch", "🔧 Utilitaires"]
)
...
with tab1:
    ...
    with col1:
        ...
        uploaded_file = st.file_uploader(
            "Choisir une image", type=["png", "jpg", "jpeg"], key="single_image"
        )
        ...
        if uploaded_file is not None:
            ...
            target_size = st.selectbox(
                "Taille cible", [(224, 224), (256, 256), (512, 512)]
            )
            ...
            if st.button("🚀 Lancer l'Analyse", type="primary"):
                with st.spinner("Analyse en cours..."):
                    ...
                    with result_tab3:
                        ...
                        st.success(
                            "✅ Cohérence entre les méthodes : Les deux pointent vers une infection COVID-19"
                        )
...
with tab2:
    ...
    st.info(
        "🔧 Cette section permet de comparer les explications de plusieurs modèles sur la même image"
    )
    ...
    with col1:
        ...
        if comparison_image:
            st.image(
                comparison_image, caption="Image à analyser", use_column_width=True
            )
        ...
        if comparison_image and models_to_compare:
            if st.button("🔄 Comparer les Modèles", type="primary"):
                with st.spinner("Comparaison en cours..."):
                    ...
                    fake_confidences = np.random.uniform(
                        0.6, 0.95, len(models_to_compare)
                    )
                    ...
                    comparison_results = pd.DataFrame(
                        {
                            ...
                            "Classe Prédite": np.random.choice(
                                class_names, len(models_to_compare)
                            ),
                            "Méthode Explication": [
                                "GradCAM" if "CNN" in m else "SHAP"
                                for m in models_to_compare
                            ],
                            "Cohérence": np.random.choice(
                                ["Élevée", "Moyenne", "Faible"], len(models_to_compare)
                            ),
                        }
                    )
...
with tab3:
    ...
    st.info(
        "🔧 Analysez plusieurs images simultanément pour générer un rapport complet"
    )
    ...
    with col1:
        ...
        if batch_files:
            ...
            batch_method = st.selectbox(
                "Méthode d'analyse", ["GradCAM", "SHAP", "Les deux"]
            )
            ...
            if st.button("🚀 Lancer Analyse Batch", type="primary"):
                ...
                with results_container:
                    ...
                    with col_chart2:
                        ...
                        confidences = [
                            float(c.strip("%")) / 100 for c in df_report["Confiance"]
                        ]
                        fig, ax = plt.subplots(figsize=(8, 6))
