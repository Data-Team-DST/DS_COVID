...
class MedicalImageAugmentor:
    ...
    def rotate_image(
        ...
        if self.preserve_aspect_ratio:
            ...
            rotated = cv2.warpAffine(
                image, rotation_matrix, (new_w, new_h), borderValue=(0, 0, 0)
            )
            ...
            rotated = cv2.warpAffine(
                image, rotation_matrix, (w, h), borderValue=(0, 0, 0)
            )
    ...
    def flip_image(
        self, image: np.ndarray, flip_type: str = "horizontal"
    ) -> np.ndarray:
        """
    ...
    def add_gaussian_noise(
        self, image: np.ndarray, noise_factor: float = 0.05
    ) -> np.ndarray:
        """
    ...
    def zoom_image(
        self, image: np.ndarray, zoom_range: Tuple[float, float] = (0.9, 1.1)
    ) -> np.ndarray:
        """
    ...
    def translate_image(
        self, image: np.ndarray, translation_range: Tuple[float, float] = (-0.1, 0.1)
    ) -> np.ndarray:
        """
        ...
        translated = cv2.warpAffine(
            image, translation_matrix, (w, h), borderValue=(0, 0, 0)
        )
    ...
    def apply_random_augmentation(
        ...
        augmentations = [
            ...
            (
                "flip",
                lambda img: self.flip_image(img) if random.random() < 0.3 else img,
            ),  # Moins fréquent
    ...
    def augment_batch(
        self, images: np.ndarray, labels: np.ndarray, augmentation_factor: int = 2
        ...
        """
        print(
            f"🔄 Augmentation de {len(images)} images (facteur: {augmentation_factor})"
        )
...
def demonstrate_augmentation_fixed(sample_images: np.ndarray, n_samples: int = 3):
    ...
    indices = np.random.choice(
        len(sample_images), min(n_samples, len(sample_images)), replace=False
    )
    ...
    augmentation_types = [
        ...
        (
            "Random Mix",
            lambda x: augmentor.apply_random_augmentation(x, max_augmentations=2),
        ),
    ]
    ...
    plt.tight_layout()
    plt.suptitle(
        "Démonstration des augmentations d'images médicales", y=1.02, fontsize=16
    )
    plt.show()
...
def balance_dataset_with_augmentation(
    ...
    for label in unique_labels:
        ...
        if current_count >= target_samples_per_class:
            ...
            indices = np.random.choice(
                current_count, target_samples_per_class, replace=False
            )
            ...
            indices = np.random.choice(
                len(aug_images), target_samples_per_class, replace=False
            )
...
try:
    ...
    __all__ = [
        ...
        "demonstrate_augmentation_fixed",
        "balance_dataset_with_augmentation",
        "CustomImageAugmenter",
    ]
except ImportError:
    __all__ = [
        "MedicalImageAugmentor",
