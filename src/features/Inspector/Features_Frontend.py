...
def _render_function_content(func_info):
    ...
    if st.button(
        f"📋 Voir le code source de {func_info['name']}",
        key=f"source_{func_info['name']}_{func_info['file']}",
    ):
...
def show_features_functions_analysis():
    ...
    try:
        ...
        if not files:
            ...
            st.info(
                "💡 Assurez-vous que le dossier `src/features/` contient des fichiers Python"
            )
            return
            ...
            if file_path:
                ...
                try:
                    ...
                    if functions_info:
                        ...
                        with col2:
                            total_lines = sum(
                                f.get("source_lines", 0) for f in functions_info
                            )
                        ...
                        with col3:
                            documented = sum(
                                ...
                                if f.get("doc", "") != "Pas de documentation"
                            )
                            st.metric(
                                "📖 Documentées", f"{documented}/{len(functions_info)}"
                            )
                        ...
                        if filtered_functions:
                            st.info(
                                f"📊 Affichage de {len(filtered_functions)}/{len(functions_info)} fonction(s)"
                            )
                            ...
                            if display_mode == "Liste compacte":
                                ...
                                for i, func_info in enumerate(filtered_functions):
                                    ...
                                    with col2:
                                        st.write(
                                            f"📏 {func_info.get('source_lines', 0)} lignes"
                                        )
                                    with col3:
                                        doc_status = (
                                            "✅"
                                    ...
                                    with col4:
                                        if st.button(
                                            "👁️ Détails",
                                            key=f"details_{i}_{func_info['name']}",
                                        ):
                                            st.session_state[
                                                f"show_details_{func_info['name']}"
                                            ] = True

                                    # Afficher les détails si demandé
                                    if st.session_state.get(
                                        f"show_details_{func_info['name']}", False
                                    ):
                                        with st.container():
                                            show_function_details(
                                                func_info, use_expander=False
                                            )
                                        if st.button(
                                            "🔼 Masquer",
                                            key=f"hide_{i}_{func_info['name']}",
                                        ):
                                            st.session_state[
                                                f"show_details_{func_info['name']}"
                                            ] = False
                            ...
                            st.warning(
                                f"⚠️ Aucune fonction ne correspond au filtre '{filter_option}'"
                            )
                    ...
                    st.error(
                        f"❌ Erreur lors de l'analyse de {selected_file}: {str(e)}"
                    )
            ...
            try:
                ...
                if summary and summary.get("all_functions"):
                        ...
                        summary_data.append(
                            {
                                ...
                                "📖 Documentée": "✅"
                                if func.get("doc", "") != "Pas de documentation"
                                else "❌",
                    ...
                    if summary_data:
                        ...
                        with col1:
                            st.metric(
                                "🔧 Total fonctions", summary.get("total_functions", 0)
                            )
                        with col2:
                            st.metric("📄 Fichiers", summary.get("total_files", 0))
                        with col3:
                            st.metric(
                                "📏 Lignes totales", summary.get("total_lines", 0)
                            )
                        with col4:
                            st.metric(
                                "📖 Documentation",
                                f"{summary.get('documented_functions', 0)}/{summary.get('total_functions', 0)}",
                            )
                        ...
                        if len(summary.get("functions_by_file", {})) > 1:
                            ...
                            chart_data = []
                            for file_name, functions in summary[
                                "functions_by_file"
                            ].items():
                                chart_data.append(
                                    {
