...
def setup_colab_environment(project_root: Path) -> bool:
    ...
    try:
        ...
        if os.path.exists("./setup.py") or os.path.exists("./pyproject.toml"):
            ...
            subprocess.run(
                ["pip", "install", "-e", ".", "--quiet"], capture_output=True
            )
...
class Config:
    ...
    classes: List[str] = field(
        default_factory=lambda: ["COVID", "Lung_Opacity", "Normal", "Viral Pneumonia"]
    )
    num_classes: int = 4
...
def get_project_config(
    env_file: Optional[Path] = None, auto_setup: bool = True
) -> Config:
    """
...
def setup_universal_environment() -> Config:
    ...
    if config.data_dir.exists():
        ...
        for cls in config.classes:
            class_paths = [config.data_dir / cls / "images", config.data_dir / cls]

            for class_path in class_paths:
                if class_path.exists():
                    images = list(class_path.glob("*.png")) + list(
                        class_path.glob("*.jpg")
                    )
                    count = len(images)
