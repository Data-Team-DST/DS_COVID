...
with st.sidebar:
    st.header("Filtres")
    show_docs_only = st.checkbox(
        "Afficher uniquement les fonctions documentées", value=False
    )
    min_lines = st.number_input(
        "Nombre minimal de lignes de code", min_value=0, value=0
    )
...
if search:
    filtered = [
        ...
        or search.lower() in (f["doc"] or "").lower()
    ]
...
if filtered:
        ...
        [
            {
                ...
                "Retour": (
                    ", ".join(f.get("return_values", ["Non détecté"]))
                    if f.get("return_values")
                    else "Non détecté"
                ),
                "Doc": (
                    "✅" if f["doc"] and f["doc"] != "Pas de documentation" else "❌"
                ),
                "Description": (
                    (f["doc"][:80] + "...")
                    if f["doc"] and len(f["doc"]) > 80
                    else (f["doc"] or "Pas de documentation")
                ),
            }
            for f in filtered
        ]
    ...
    idx = st.number_input(
        "Index de la fonction à détailler",
        min_value=0,
    ...
    if func.get("return_values"):
        ...
        st.markdown("**Valeurs de retour :** Non détecté")
    ...
    if func["parameters"]:
        ...
        for p in func["parameters"]:
            st.write(
                f"- `{p['name']}` : {p.get('annotation', 'Type non spécifié')} (Défaut : {p.get('default', 'Requis')})"
            )
