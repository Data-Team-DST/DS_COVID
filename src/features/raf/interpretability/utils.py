...
class InterpretabilityAnalyzer:
    ...
    def analyze_cnn_prediction(
        ...
        for method in methods:
            try:
                if method == "gradcam":
                    results[method] = self.gradcam_explainer.generate_gradcam(
                        img, class_idx
                    )
                elif method == "gradcam++":
                    results[method] = self.gradcam_explainer.generate_gradcam_plus_plus(
                        img, class_idx
                    )
        ...
        if results:
            comparison_fig = visualize_gradcam_comparison(
                results, "Comparaison des méthodes GradCAM", class_names
            )
    ...
    def analyze_ml_prediction(
        ...
        try:
            ...
            results = {
                ...
                "waterfall_fig": self.shap_explainer.plot_waterfall(
                    instance_idx, feature_names=feature_names, show=False
                ),
                "summary_fig": self.shap_explainer.plot_summary(
    ...
    def compare_predictions(
        ...
        if self.shap_explainer is not None:
            try:
                ...
                ml_results = self.analyze_ml_prediction(
                    X_background, X_explain, feature_names
                )
...
def create_interpretability_dashboard(
    ...
    results = analyzer.compare_predictions(
        img, X_flat, X_background, class_names, feature_names
    )
    ...
    # Titre principal
    fig.suptitle(
        "Tableau de Bord d'Interprétabilité - COVID-19", fontsize=16, fontweight="bold"
    )
...
• >90%: {features["concentration_90"]:.1%}"""
    ...
    if "ml" in results and "shap_values" in results["ml"]:
        ...
        bars = ax5.barh(
            range(len(importance_df)), importance_df["importance"], alpha=0.7
        )
        ...
        try:
            ...
            if analyzer.gradcam_explainer is not None:
                ...
                if class_names and gradcam_result["predicted_class"] < len(class_names):
                    row_data["predicted_class_name"] = class_names[
...
def plot_interpretability_summary(
    gradcam_results: List[Dict], class_names: Optional[List[str]] = None
) -> plt.Figure:
    """
