...
tab1, tab2, tab3 = st.tabs(
    ["🎯 Prédiction Simple", "📊 Batch Prédiction", "📈 Analyse"]
)
    ...
    if model_source == "Session courante":
        if "training_results" in st.session_state:
            ...
            if model_names:
                ...
                st.warning(
                    "⚠️ Aucun modèle disponible en session. Entraînez d'abord un modèle."
                )
        else:
        ...
        if prediction_mode == "Données simulées":
                ...
                with st.spinner("🔄 Génération des données et prédiction..."):
                    try:
                        ...
                        results_df = pd.DataFrame(
                            {
                                ...
                                "Confiance": [
                                    f"{max(prob):.2%}" for prob in probabilities
                                ],
                            }
                        )
                        ...
                        for i in range(probabilities.shape[1]):
                            results_df[f"Prob Classe {i}"] = [
                                f"{prob[i]:.3f}" for prob in probabilities
                            ]
            ...
            if uploaded_file is not None:
                try:
                    ...
                    st.success(
                        f"✅ Fichier chargé: {df_upload.shape[0]} lignes, {df_upload.shape[1]} colonnes"
                    )
            ...
            st.info(
                "🚧 Saisie manuelle à implémenter selon le type de données spécifique"
            )
            ...
            with st.expander("🔧 Interface de saisie (exemple)"):
                ...
                if st.button("🔮 Prédire"):
                    st.info(
                        "Interface de saisie manuelle à adapter selon vos données spécifiques"
                    )
...
with tab2:
    ...
    if selected_model is not None:
        ...
        batch_mode = st.selectbox(
            "Mode de traitement", ["Dossier de fichiers", "Multiple fichiers CSV"]
        )
...
with tab3:
    ...
    if "last_predictions" in st.session_state:
        ...
        with col2:
            confidence_mean = np.mean(
                [max(prob) for prob in last_pred["probabilities"]]
            )
            st.metric("Confiance Moyenne", f"{confidence_mean:.2%}")
            ...
            freq_df = pd.DataFrame(
                {
                    ...
                    "Pourcentage": [
                        f"{(count/sum(pred_counts.values)*100):.1f}%"
        ...
        if low_conf_indices:
            st.warning(
                f"⚠️ {len(low_conf_indices)} échantillon(s) avec confiance < {low_confidence_threshold}"
            )
            ...
            low_conf_df = pd.DataFrame(
                {
                    ...
                    "Prédiction": [
                        last_pred["predictions"][i] for i in low_conf_indices
                    ],
            ...
            st.success(
                f"✅ Tous les échantillons ont une confiance ≥ {low_confidence_threshold}"
            )
