...
def get_analysis_mode_config(context="default"):
    ...
    with col2:
        ...
        enhanced_analysis = st.checkbox(
            "🚀 Analyse améliorée", value=False, key=f"enhanced_{context}"
        )
    ...
    if analysis_mode == "🚀 Rapide (échantillon)":
        config["sample_size"] = st.slider(
            "Nombre d'échantillons:", 5, 100, 20, key=f"rapid_sample_{context}"
        )
        ...
        config["confirm"] = st.checkbox(
            "Je confirme vouloir analyser toutes les images",
            key=f"full_confirm_{context}",
        )
        ...
        percentage = st.slider(
            ...
            50,
            step=10,
            key=f"balanced_percent_{context}",
        )
...
def get_image_files_for_analysis(
    data_dir: Path, category: str, config: Dict
) -> List[Path]:
...
def analyze_images_batch(image_paths: List[Path], batch_size: int = 10) -> Dict:
    ...
    results = {"sizes": [], "means": [], "stds": [], "file_sizes": [], "formats": []}
    ...
    for i in range(0, total_files, batch_size):
        ...
        for img_path in batch:
            ...
            if processed > 0:
                ...
                status_text.text(
                    f"Traitement: {processed}/{total_files} images - ETA: {eta:.1f}s"
                )
...
def analyze_images_batch_enhanced(
    image_paths: List[Path], batch_size: int = 10
) -> Dict:
        ...
        for img_path in batch:
            try:
                with Image.open(img_path) as img:
                    ...
                    results["contrast_ratio"].append(
                        calculate_contrast_ratio(img_array)
                    )
...
def show_radiological_analysis(data_dir, categories):
    ...
    sample_per_category = st.slider(
        "Échantillons par catégorie:", 5, 30, 10, key="radio_samples"
    )
...
def show_basic_image_analysis(data_dir, categories):
    ...
    with col1:
        selected_category = st.selectbox(
            "Catégorie à analyser:", categories, key="enhanced_analysis_category"
        )
    ...
    with col2:
        ...
        if images_dir.exists():
            ...
            files_to_process = get_image_files_for_analysis(
                Path(data_dir), selected_category, config
            )
            st.info(
                f"📂 **{len(files_to_process)}** images seront analysées sur **{total_images}** disponibles"
            )
        else:
    ...
    if st.button("🚀 Lancer l'analyse", key="enhanced_analysis_btn", type="primary"):
        with st.spinner("🔍 Analyse en cours..."):
            ...
            image_files = get_image_files_for_analysis(
                Path(data_dir), selected_category, config
            )
            ...
            st.success(
                f"✅ Analyse de **{len(image_files)}** images de la catégorie **{selected_category}**"
            )
                ...
                stats_df = pd.DataFrame(
                    {
                        ...
                        "Taille_Fichier_KB": [
                            size / 1024 for size in results["file_sizes"]
                        ],
                    }
                )
...
def show_random_image_grid(data_dir, categories):
    ...
    with col1:
        if st.button(
            ...
            key="generate_random_samples",
        ):
            st.rerun()

    with col2:
        grid_size = st.selectbox(
            "Taille de la grille:", [2, 3, 4], index=1, key="random_grid_size"
        )
...
def calculate_glcm_correlation(img_array):
    ...
    try:
        ...
        valid_correlations = [
            c for c in correlations if not np.isnan(c) and not np.isinf(c)
        ]
...
def calculate_dominant_orientation(img_array):
    ...
    angles = angles[
        np.abs(grad_x) + np.abs(grad_y) > np.mean(np.abs(grad_x) + np.abs(grad_y))
    ]
...
def calculate_circularity(img_array):
    ...
    if np.sum(edges) > 0:
        ...
        if len(x) > 0 and len(y) > 0:
            ...
            return (
                1.0 / (1.0 + np.std(distances) / np.mean(distances))
...
def calculate_frequency_distribution(magnitude_spectrum):
    ...
    if total_energy > 0:
        ...
        for i in range(bins):
            mask = (distances >= i * max_dist / bins) & (
                distances < (i + 1) * max_dist / bins
            )
...
def detect_abnormal_texture_patterns(img_array):
    ...
    abnormal_score = (local_entropy > entropy_threshold) + (
...
def display_comparative_summary(combined_df, selected_categories):
        ...
        for category in selected_categories:
            ...
            if not cat_data.empty:
                ...
                if "Score_Qualite_Global" in cat_data.columns:
                    summary_row["Qualité moyenne"] = (
                        f"{cat_data['Score_Qualite_Global'].mean():.2f}"
                    )
                if "Nettet_Gradient" in cat_data.columns:
                    summary_row["Netteté moyenne"] = (
                        f"{cat_data['Nettet_Gradient'].mean():.2f}"
                    )
                if "Contraste_Michelson" in cat_data.columns:
                    summary_row["Contraste moyen"] = (
                        f"{cat_data['Contraste_Michelson'].mean():.3f}"
                    )
    ...
    quantitative_columns = [
        ...
        if col not in ["Pixels_Total", "Taille_Fichier_KB"]
    ]
            ...
            if st.button("🧮 Effectuer ANOVA", key="perform_anova"):
                try:
                    ...
                    if len(groups) >= 2:
                        ...
                        if p_value < 0.05:
                            st.success(
                                "✅ Différence significative entre les catégories (p < 0.05)"
                            )
                        else:
                            st.info(
                                "ℹ️ Pas de différence significative entre les catégories (p ≥ 0.05)"
                            )
                    else:
...
def display_global_comparative_analysis(combined_df, analysis_options):
    ...
    with comp_tabs[0]:  # Comparaison Générale
        ...
        numeric_columns = [
            col for col in numeric_columns if col not in ["Pixels_Total"]
        ]

        if len(numeric_columns) > 0:
            stats_by_category = combined_df.groupby("Categorie")[numeric_columns].agg(
                ["mean", "std", "min", "max"]
            )
                ...
                for metric in quality_metrics:
                    if normalized_data[metric].std() > 0:
                        normalized_data[metric] = (
                            ...
                            normalized_data[metric].max()
                            - normalized_data[metric].min()
                        )
        ...
        if "🎨 Analyse de texture avancée" in str(analysis_options):
            ...
            if texture_metrics:
                ...
                if len(texture_data) > 0 and len(texture_metrics) > 1:
                    ...
                    pca_df = pd.DataFrame(
                        texture_pca,
                        columns=[f"PC{i+1}" for i in range(texture_pca.shape[1])],
                    )
                    ...
                    # Variance expliquée
                    st.write(
                        f"**Variance expliquée par les composantes:** {pca.explained_variance_ratio_.round(3)}"
                    )
        ...
        if len(numeric_cols) > 1:
            ...
            if selected_category_corr == "Toutes":
                ...
                corr_data = combined_df[
                    combined_df["Categorie"] == selected_category_corr
                ][numeric_cols]
        ...
        if len(numeric_cols) >= 2:
            ...
            with col1:
                metric_x = st.selectbox(
                    "Métrique X:", numeric_cols, key="corr_metric_x"
                )

            with col2:
                metric_y = st.selectbox(
                    "Métrique Y:", numeric_cols, index=1, key="corr_metric_y"
                )
...
def display_comprehensive_results(
    results_df, analysis_options, category, processing_times
):
...
def display_quality_results(results_df):
    ...
    quality_cols = [
        ...
        if any(
            keyword in col
            for keyword in ["Nettete", "Contraste", "Bruit", "Blur", "Score_Qualite"]
        )
    ]
    ...
    if quality_cols:
        ...
        if len(quality_cols) >= 2:
            ...
            with col1:
                if (
                    "Nettete_Laplacian" in results_df.columns
                    and "Contraste_RMS" in results_df.columns
                ):
                    fig = px.scatter(
                        results_df,
                        x="Nettete_Laplacian",
            ...
            with col2:
                if (
                    "Score_Qualite_Global" in results_df.columns
                    and "Niveau_Bruit" in results_df.columns
                ):
                    fig = px.scatter(
                        results_df,
                        x="Niveau_Bruit",
        ...
        if len(quality_cols) >= 3:
            ...
            available_radar = [
                col for col in radar_metrics if col in results_df.columns
            ]
            ...
            if len(available_radar) >= 3:
                ...
                for metric in available_radar:
                    if results_df[metric].max() != results_df[metric].min():
                        radar_data[metric] = (
                            results_df[metric].mean() - results_df[metric].min()
    ...
    if texture_cols:
        ...
        with col1:
            if "Entropie" in results_df.columns and "Homogeneite" in results_df.columns:
                ...
                line_x = np.linspace(
                    results_df["Entropie"].min(), results_df["Entropie"].max(), 100
                )
        ...
        with col2:
            if (
                "Variance_Locale" in results_df.columns
                and "Contraste_GLCM" in results_df.columns
            ):
                fig = px.scatter(
                    results_df,
                    x="Variance_Locale",
...
def display_edge_results(results_df):
    ...
    if edge_cols:
        ...
        with col2:
            if (
                "Circularite" in results_df.columns
                and "Rectangularite" in results_df.columns
            ):
                fig = px.scatter(
                    results_df,
                    x="Circularite",
        ...
        if "Orientation_Dominante" in results_df.columns:
            ...
            orientations = (
                results_df["Orientation_Dominante"] * 180 / np.pi
            )  # Convertir en degrés
...
def display_statistical_results(results_df):
    ...
    if stats_cols:
        ...
        with col1:
            if (
                "Asymetrie" in results_df.columns
                and "Aplatissement" in results_df.columns
            ):
                fig = px.scatter(
                    results_df,
                    x="Asymetrie",
                ...
                fig.add_hline(
                    y=0, line_dash="dash", line_color="gray", annotation_text="Normale"
                )
                fig.add_vline(
                    x=0,
                    line_dash="dash",
                    line_color="gray",
                    annotation_text="Symétrique",
                )
                st.plotly_chart(fig, use_container_width=True)
        ...
        with col2:
            if "Test_Normalite_Shapiro" in results_df.columns:
                ...
                fig.add_vline(
                    x=0.05, line_dash="dash", line_color="red", annotation_text="α=0.05"
                )
                st.plotly_chart(fig, use_container_width=True)

        # Distribution des percentiles
        if (
            "Percentile_5" in results_df.columns
            and "Percentile_95" in results_df.columns
        ):
...
def display_medical_results(results_df):
    ...
    if medical_cols:
        ...
        with col2:
            # Analyse de symétrie
            if (
                "Symetrie_Horizontale" in results_df.columns
                and "Symetrie_Verticale" in results_df.columns
            ):
                fig = px.scatter(
                    results_df,
        ...
        if len(medical_cols) >= 4:
            ...
            radar_medical = [
                ...
                "Score_Texture_Anormale",
                "Homogeneite_Regionale",
            ]
            available_medical = [
                col for col in radar_medical if col in results_df.columns
            ]
            ...
            if len(available_medical) >= 3:
                ...
                for metric in available_medical:
                    if results_df[metric].max() != results_df[metric].min():
                        if (
                            "Anormale" in metric
                        ):  # Inverser pour les métriques "mauvaises"
                            radar_data[metric] = 1 - (
                                results_df[metric].mean() - results_df[metric].min()
                            ) / (results_df[metric].max() - results_df[metric].min())
                        else:
...
def display_color_results(results_df):
    ...
    if color_cols:
        ...
        with col1:
            ...
            if all(
                col in results_df.columns
                for col in ["Dominance_Rouge", "Dominance_Vert", "Dominance_Bleu"]
            ):
                avg_red = results_df["Dominance_Rouge"].mean()
                avg_green = results_df["Dominance_Vert"].mean()
                avg_blue = results_df["Dominance_Bleu"].mean()
        ...
        if (
            "Saturation_Moyenne" in results_df.columns
            and "Luminosite_Moyenne" in results_df.columns
        ):
            col1, col2 = st.columns(2)
...
def display_frequency_results(results_df):
    ...
    freq_cols = [
        ...
        if any(
            keyword in col
            for keyword in ["Energie_Spectrale", "Frequence", "Periodicite"]
        )
    ]
    ...
    if freq_cols:
        ...
        with col4:
            if "Frequence_Dominante" in results_df.columns:
                ...
                freq_str = (
                    str(results_df["Frequence_Dominante"].iloc[0])
                    if len(results_df) > 0
                    else "N/A"
                )
        ...
        with col2:
            if (
                "Periodicite_Horizontale" in results_df.columns
                and "Periodicite_Verticale" in results_df.columns
            ):
                fig = px.scatter(
                    results_df,
                ...
                max_val = max(
                    results_df["Periodicite_Horizontale"].max(),
                    results_df["Periodicite_Verticale"].max(),
                )
                fig.add_shape(
                    type="line",
                    x0=0,
        ...
        if "Repartition_Frequences" in results_df.columns:
            ...
            st.info(
                "📊 Les données de répartition spectrale sont disponibles mais nécessitent un décodage spécialisé"
            )
...
def display_ai_results(results_df):
    ...
    if ai_cols:
        ...
        with col1:
            if (
                "Complexite_Kolmogorov" in results_df.columns
                and "Dimension_Fractale" in results_df.columns
            ):
                fig = px.scatter(
                    results_df,
        ...
        with col2:
            if (
                "Predictibilite" in results_df.columns
                and "Complexite_Visuelle" in results_df.columns
            ):
                fig = px.scatter(
                    results_df,
                    x="Predictibilite",
        ...
        if len(ai_cols) >= 4:
            ...
            ai_radar_metrics = [
                "Complexite_Kolmogorov",
                "Dimension_Fractale",
                "Entropie_Renyi",
                "Complexite_Visuelle",
            ]
            available_ai = [
                col for col in ai_radar_metrics if col in results_df.columns
            ]
            ...
            if len(available_ai) >= 3:
                ...
                fig.add_trace(
                    go.Scatterpolar(
                        r=list(radar_data.values()),
                        theta=[
                            m.replace("_", " ").replace("Complexite", "Cpx")
                            for m in available_ai
                        ],
                        fill="toself",
        ...
        if "Complexite_Visuelle" in results_df.columns:
            ...
            fig.add_vline(
                x=0.33, line_dash="dash", line_color="green", annotation_text="Faible"
            )
            fig.add_vline(
                x=0.66, line_dash="dash", line_color="orange", annotation_text="Élevée"
            )
            st.plotly_chart(fig, use_container_width=True)
    ...
    with col2:
        if st.button("📄 Générer Rapport JSON"):
            report = {
                ...
                "summary": {
                    "avg_quality": results_df.get(
                        "Score_Qualite_Global", pd.Series([0])
                    ).mean(),
                    "avg_complexity": results_df.get(
                        "Complexite_Visuelle", pd.Series([0])
                    ).mean(),
                },
            }
...
def show_advanced_image_metrics(data_dir, categories):
    ...
    with col2:
        ...
        max_images_per_category = st.slider(
            "📷 Images par catégorie:", 5, 100, 20, key="advanced_metrics_sample_size"
        )
        show_progress_details = st.checkbox(
            "📊 Afficher les détails de progression", value=True
        )
    ...
    with st.expander("🔧 **Options Avancées**"):
        ...
        with col1:
            save_intermediate = st.checkbox(
                "💾 Sauvegarder résultats intermédiaires", value=False
            )
            compare_with_reference = st.checkbox(
                "📏 Comparer avec image de référence", value=False
            )

        with col2:
            export_visualizations = st.checkbox(
                "📈 Exporter visualisations", value=False
            )
            generate_report = st.checkbox("📄 Générer rapport PDF", value=False)

        with col3:
            batch_processing = st.checkbox(
                "⚡ Traitement par lots optimisé", value=True
            )
        ...
        if ref_images_dir.exists():
            ...
            if ref_images:
                ...
                try:
                    ...
                    st.image(
                        ref_img,
                        caption=f"Image de référence: {ref_image_name}",
                        width=200,
                    )
                except Exception as e:
    ...
    total_images = sum(
        [
            len(list((Path(data_dir) / cat / "images").glob("*.png")))
            for cat in selected_categories
            if (Path(data_dir) / cat / "images").exists()
        ]
    )
    estimated_time = (
        min(total_images, len(selected_categories) * max_images_per_category)
        * len(analysis_options)
        * 0.1
    )

    if estimated_time > 60:
        st.warning(
            f"⏱️ Temps estimé: {estimated_time/60:.1f} minutes. Considérez réduire le nombre d'images ou d'analyses."
        )

    # Bouton de lancement amélioré
    if st.button(
        "🚀 **Lancer l'Analyse Avancée Complète**",
        key="launch_comprehensive_analysis",
        type="primary",
    ):
        ...
        for cat_idx, category in enumerate(selected_categories):
            global_status.text(
                f"🔍 Analyse de la catégorie: {category} ({cat_idx+1}/{total_categories})"
            )
            ...
            ):  # Affichage détaillé pour une seule catégorie
                st.markdown("---")
            ...
            for i, image_path in enumerate(image_files):
                ...
                try:
                    if len(selected_categories) == 1 and show_progress_details:
                        status_text.text(
                            f"🔍 Analyse en cours: {image_path.name} ({i+1}/{len(image_files)})"
                        )
                    ...
                    for analysis_type in analysis_options:
                        ...
                        try:
                            if "🏆 Qualité d'image complète" in analysis_type:
                                ...
                                ref_img = (
                                    reference_img_array
                                    if "reference_img_array" in locals()
                                    else None
                                )
                                quality_results = (
                            ...
                            if "🎨 Analyse de texture avancée" in analysis_type:
                                texture_results = perform_advanced_texture_analysis(
                                    img_array
                                )
                                image_results.update(texture_results)

                            if "🔲 Détection de contours et formes" in analysis_type:
                                edge_results = perform_comprehensive_edge_analysis(
                                    img_array
                                )
                                image_results.update(edge_results)

                            if "📊 Analyse statistique approfondie" in analysis_type:
                                stats_results = perform_deep_statistical_analysis(
                                    img_array
                                )
                                image_results.update(stats_results)

                            if (
                                "🌈 Analyse des couleurs" in analysis_type
                                and len(img_array_color.shape) == 3
                            ):
                            ...
                            if "🏥 Métriques médicales" in analysis_type:
                                medical_results = perform_medical_metrics_analysis(
                                    img_array
                                )
                            ...
                            category_processing_times[f"{analysis_type}_{i}"] = (
                                time.time() - analysis_start
                            )

                        except Exception as analysis_error:
                            st.warning(
                                f"⚠️ Erreur lors de l'analyse '{analysis_type}' pour {image_path.name}: {str(analysis_error)}"
                            )
                    ...
                    if len(selected_categories) == 1 and show_progress_details:
                        ...
                        if "Score_Qualite_Global" in image_results:
                            quality_metric.metric(
                                "🏆 Qualité Moy.",
                                f"{image_results['Score_Qualite_Global']:.2f}",
                            )
        ...
        if all_category_results:
            st.success(
                f"✅ **Analyse terminée en {total_processing_time:.2f}s** pour {len(selected_categories)} catégorie(s)"
            )
                ...
                if all_results_list:
                    ...
                    for i, category in enumerate(selected_categories):
                        with category_tabs[i]:
                            if (
                                category in all_category_results
                                and all_category_results[category]
                            ):
...
def perform_comprehensive_quality_analysis(img_array, reference_img=None):
    ...
    try:
        ...
        contrast_score = np.mean(
            [results.get("Contraste_RMS", 0), results.get("Contraste_Michelson", 0)]
        )

        # Normalisation du score final
        quality_raw = sharpness_score * noise_penalty * contrast_score
        results["Score_Qualite_Global"] = min(
            max(quality_raw / 1000, 0), 1
...
def perform_advanced_texture_analysis(img_array):
    ...
    try:
        ...
        # Analyse de motifs
        try:
            results["Repetitivite_Texture"] = calculate_texture_repetitiveness(
                img_array
            )
        except Exception:
...
def perform_deep_statistical_analysis(img_array):
    ...
    results["Coefficient_Variation"] = (
        results["Ecart_Type"] / results["Moyenne"] if results["Moyenne"] > 0 else 0
    )
...
def perform_color_analysis(img_array_color):
    ...
    if len(img_array_color.shape) == 3:
        ...
        results["Nombre_Couleurs_Uniques"] = len(
            np.unique(img_array_color.reshape(-1, 3), axis=0)
        )
...
def perform_frequency_analysis(img_array):
    ...
    results["Repartition_Frequences"] = calculate_frequency_distribution(
        magnitude_spectrum
    )
...
def perform_medical_metrics_analysis(img_array):
    ...
    results["Regions_Moyennes_Pct"] = (
        100 - results["Regions_Sombres_Pct"] - results["Regions_Claires_Pct"]
    )
...
def display_global_comparison_enhanced(
    all_results: Dict, selected_categories: List[str]
):
                ...
                detailed_data.append(
                    {
                        ...
                        "Entropie": (
                            results.get("entropy", [0])[i]
                            if i < len(results.get("entropy", []))
                            else 0
                        ),
                        "Netteté": (
                            results.get("sharpness", [0])[i]
                            if i < len(results.get("sharpness", []))
                            else 0
                        ),
                        "Contraste": (
                            results.get("contrast_ratio", [0])[i]
                            if i < len(results.get("contrast_ratio", []))
                            else 0
                        ),
                    }
                )
    ...
    if comparison_data:
        ...
        with col2:
            avg_resolution = (
                comparison_df["Largeur_Moyenne"].mean()
                * comparison_df["Hauteur_Moyenne"].mean()
            )
        ...
        with col4:
            category_with_best_quality = comparison_df.loc[
                comparison_df["Netteté_Moyenne"].idxmax(), "Catégorie"
            ]
        ...
        with col1:
            ...
            groups = [
                detailed_df[detailed_df["Catégorie"] == cat]["Intensité"].values
                for cat in selected_categories
            ]
        ...
        with col2:
            ...
            intensity_stats = detailed_df.groupby("Catégorie")["Intensité"].agg(
                ["mean", "std", "min", "max"]
            )
...
def create_radar_chart(comparison_df):
    ...
    for metric in available_metrics:
        if radar_data[metric].max() != radar_data[metric].min():
            radar_data[metric + "_norm"] = (
                radar_data[metric] - radar_data[metric].min()
    ...
    for _, row in radar_data.iterrows():
        fig.add_trace(
            go.Scatterpolar(
                ...
                theta=[
                    m.replace("_Moyenne", "").replace("_Moyen", "")
...
def generate_insights(comparison_df, detailed_df):
    ...
    insights.append(
        f"📷 **{max_images_cat['Catégorie']}** contient le plus d'images ({max_images_cat['Nombre_Images']:,})"
    )
    ...
    if "Netteté_Moyenne" in comparison_df.columns:
        ...
        insights.append(
            f"🏆 **{best_quality_cat['Catégorie']}** a la meilleure netteté moyenne ({best_quality_cat['Netteté_Moyenne']:.2f})"
        )

    # Insight 3: Plus grande variabilité
    most_variable_cat = comparison_df.loc[
        comparison_df["Écart_Type_Intensité"].idxmax()
    ]
    insights.append(
        f"📊 **{most_variable_cat['Catégorie']}** présente la plus grande variabilité d'intensité"
    )

    # Insight 4: Taille de fichier
    largest_files_cat = comparison_df.loc[
        comparison_df["Taille_Fichier_Moyenne_KB"].idxmax()
    ]
    insights.append(
        f"💾 **{largest_files_cat['Catégorie']}** a les fichiers les plus volumineux ({largest_files_cat['Taille_Fichier_Moyenne_KB']:.1f} KB)"
    )
    ...
    if categories_count >= 3:
        insights.append(
            f"🔍 Analyse comparative sur **{categories_count} catégories** avec des caractéristiques distinctes"
        )
    ...
    if "Netteté_Moyenne" in comparison_df.columns:
        low_quality_cats = comparison_df[
            comparison_df["Netteté_Moyenne"] < comparison_df["Netteté_Moyenne"].mean()
        ]
        if not low_quality_cats.empty:
            st.warning(
                f"⚠️ Considérer l'amélioration de la qualité pour: {', '.join(low_quality_cats['Catégorie'].tolist())}"
            )
    ...
    if max_images / min_images > 2:
        st.warning(
            f"⚖️ Dataset déséquilibré détecté (ratio {max_images/min_images:.1f}:1)"
        )
...
def show_global_analysis(data_dir, categories):
    ...
    if not categories:
        st.warning(
            "⚠️ Aucune catégorie sélectionnée. Veuillez sélectionner au moins une catégorie."
        )
        return
    ...
    st.info(
        f"🎯 **Mode d'analyse**: {analysis_config['name']} | "
    ...
    # Estimation du temps de traitement
    total_estimated_time = estimate_processing_time(
        data_dir, selected_categories, analysis_config
    )
    if total_estimated_time > 30:
        st.warning(
            f"⏱️ Temps estimé: {total_estimated_time:.1f}s. Considérez le mode échantillon pour un traitement plus rapide."
        )
    ...
    if st.button("🚀 Lancer l'analyse globale", key="start_global_analysis"):
                ...
                if category_dir.exists():
                    ...
                    if image_files:
                        if analysis_config["enhanced"]:
                            results = analyze_images_batch_enhanced(
                    ...
                    processed_metric.metric(
                        "📷 Images traitées", f"{total_processed:,}"
                    )
        ...
        if all_results:
            # Métriques finales
            st.success(
                f"✅ Analyse terminée en {total_time:.1f}s pour {len(selected_categories)} catégorie(s)"
            )
            ...
            with final_col2:
                st.metric(
                    "⚡ Vitesse moyenne", f"{total_processed/total_time:.1f} img/s"
                )
            with final_col3:
            ...
            comparison_df, detailed_df = display_global_comparison_enhanced(
