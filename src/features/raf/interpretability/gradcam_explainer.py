...
class GradCAMExplainer:
    ...
    def generate_gradcam(
        ...
        if len(original_img.shape) == 3 and original_img.shape[-1] == 3:
            ...
            original_for_overlay = np.repeat(
                (original_img[:, :, 0] * 255).astype(np.uint8)[:, :, np.newaxis],
                3,
                axis=2,
            )
        else:
            ...
            original_for_overlay = np.repeat(
                (original_img * 255).astype(np.uint8)[:, :, np.newaxis], 3, axis=2
            )

        # Superposition
        overlay = cv2.addWeighted(
            original_for_overlay, 1 - alpha, heatmap_colored, alpha, 0
        )
    ...
    def generate_gradcam_plus_plus(
        ...
        alpha_num = grads_2
        alpha_denom = 2.0 * grads_2 + tf.reduce_sum(
            grads_3 * global_sum, axis=(1, 2), keepdims=True
        )
    ...
    def analyze_multiple_layers(
        self, img: np.ndarray, layer_names: List[str], class_idx: Optional[int] = None
...
def analyze_gradcam_regions(
    heatmap: np.ndarray, threshold: float = 0.5
) -> Dict[str, Any]:
    """
