...
tab1, tab2, tab3, tab4 = st.tabs(
    ["🎯 Sélection Modèles", "📊 Métriques", "📈 Visualisations", "📋 Rapport"]
)
                    ...
                    with st.expander(f"🔍 {model_name}", expanded=False):
                        ...
                        with col3:
                            ...
                            if training_time:
                                st.metric(
                                    "Temps entraînement",
                                    f"{training_time.total_seconds():.1f}s",
                                )
...
with tab2:
    ...
    if (
        eval_mode == "Modèles en session"
        and "training_results" in st.session_state
        and selected_models
    ):
        results = st.session_state["training_results"]
        ...
        for model_name in selected_models:
            ...
            if "cv_scores" in result:
                for score in result["cv_scores"]:
                    cv_data.append({"Modèle": model_name, "Score CV": score})
...
with tab4:
    ...
    if (
        ...
        best_model = max(
            selected_models, key=lambda x: results[x].get("test_accuracy", 0)
        )
...
"""
        ...
        times = [
            results[name].get("training_time", pd.Timedelta(0)).total_seconds()
        ...
        if max(accuracies) - min(accuracies) < 0.05:
            recommendations.append(
                "- Les performances sont similaires entre modèles. Privilégier le plus rapide."
            )
        ...
        if len(selected_models) > 1:
            recommendations.append(
                "- Considérer un ensemble (ensemble) des meilleurs modèles."
            )
                    ...
                    metrics_export.append(
                        {
                            ...
                            "training_time_s": result.get(
                                "training_time", pd.Timedelta(0)
                            ).total_seconds(),
                        }
                    )
