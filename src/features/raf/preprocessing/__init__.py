...
class ImagePreprocessor:
    ...
    def resize_image(
        self, image: np.ndarray, maintain_aspect_ratio: bool = False
    ) -> np.ndarray:
        """
        ...
        for filter_name in filters:
            if filter_name == "gaussian_blur":
                ...
                filtered_image = cv2.bilateralFilter(
                    filtered_image.astype(np.uint8), 9, 75, 75
                )
                ...
                if len(filtered_image.shape) == 3:
                    gray = cv2.cvtColor(
                        filtered_image.astype(np.uint8), cv2.COLOR_RGB2GRAY
                    )
                else:
                ...
                filtered_image = cv2.addWeighted(
                    filtered_image.astype(np.uint8), 0.8, edges, 0.2, 0
                )
...
def prepare_data_splits(
    ...
    print(
        f"   Train: {X_train.shape[0]} échantillons ({X_train.shape[0] / X.shape[0] * 100:.1f}%)"
    )
    print(
        f"   Val:   {X_val.shape[0]} échantillons ({X_val.shape[0] / X.shape[0] * 100:.1f}%)"
    )
    print(
        f"   Test:  {X_test.shape[0]} échantillons ({X_test.shape[0] / X.shape[0] * 100:.1f}%)"
    )
...
def scale_features(
    X_train: np.ndarray, X_val: np.ndarray, X_test: np.ndarray, method: str = "standard"
    ...
    print(
        f"   Moyennes train: min={X_train_scaled.mean(axis=0).min():.3f}, max={X_train_scaled.mean(axis=0).max():.3f}"
    )
    print(
        f"   Std train: min={X_train_scaled.std(axis=0).min():.3f}, max={X_train_scaled.std(axis=0).max():.3f}"
    )
...
def remove_outliers(
    X: np.ndarray, y: np.ndarray, method: str = "iqr", contamination: float = 0.1
    ...
    print(
        f"   Outliers détectés: {n_outliers} ({n_outliers / n_samples_original * 100:.1f}%)"
    )
    print(
        f"   Échantillons gardés: {n_samples_clean} ({n_samples_clean / n_samples_original * 100:.1f}%)"
    )
