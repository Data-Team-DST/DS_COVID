...
def run():
    ...
    st.image(str(image_path), caption="Support Vector Machine (SVM)", use_column_width=False)
    st.markdown("""**k-Nearest Neighbors (k-NN)** :""")
    chemin_absolu = rf"{chemin_global}\page\KNN.png"
    image_path = Path(chemin_absolu).relative_to(Path.cwd())
    st.image(str(image_path), caption="k-Nearest Neighbors (k-NN)", use_column_width=False)
    st.markdown("""**Random Forest** :""")
    chemin_absolu = rf"{chemin_global}\page\random forest.png"
    image_path = Path(chemin_absolu).relative_to(Path.cwd())
    st.image(str(image_path), caption="Random Forest", use_column_width=False)
    ...
    image_path = Path(chemin_absolu).relative_to(Path.cwd())
    st.image(str(image_path), caption="Matrice de confusion", use_column_width=False)
    chemin_absolu = rf"{chemin_global}\page\interprétation des résultats.png"
    image_path = Path(chemin_absolu).relative_to(Path.cwd())
    st.image(str(image_path), caption="Interprétation des résultats", use_column_width=False)
    st.markdown("""**Modèle de deep learning Inception V3** :""")
    chemin_absolu = rf"{chemin_global}\page\Inceptionv3.png"
    image_path = Path(chemin_absolu).relative_to(Path.cwd())
    st.image(str(image_path), caption="Schéma explicatif", use_column_width=False)
    chemin_absolu = rf"{chemin_global}\page\courbe de loss et d'accuracy.png"
    image_path = Path(chemin_absolu).relative_to(Path.cwd())
    st.image(str(image_path), caption="Courbe de loss et d'accuracy", use_column_width=False)
