...
def analyze_image_properties(
    images: np.ndarray, labels: Optional[np.ndarray] = None, sample_size: int = 1000
) -> Dict[str, Any]:
    """
    ...
    print(
        f"   Min/Max: {stats['pixel_stats']['min']:.3f} / {stats['pixel_stats']['max']:.3f}"
    )
...
def visualize_sample_images(
    ...
    title: str = "Échantillon d'images",
) -> None:
    """
...
def check_data_quality(images: np.ndarray, labels: np.ndarray) -> Dict[str, Any]:
    ...
    quality_report = {"issues": [], "warnings": [], "info": []}

    # Vérification des dimensions
    if len(images) != len(labels):
        quality_report["issues"].append(
            f"Désalignement: {len(images)} images vs {len(labels)} labels"
        )
    ...
    if inf_images > 0:
        quality_report["issues"].append(
            f"{inf_images} images contiennent des valeurs infinies"
        )
    ...
    if max_class_size / min_class_size > 10:
        quality_report["warnings"].append(
            f"Dataset très déséquilibré (ratio {max_class_size / min_class_size:.1f}:1)"
        )
    elif max_class_size / min_class_size > 3:
        quality_report["warnings"].append(
            f"Dataset déséquilibré (ratio {max_class_size / min_class_size:.1f}:1)"
        )

    # Informations générales
    quality_report["info"].append(
        f"Total: {len(images)} images, {len(unique_labels)} classes"
    )
