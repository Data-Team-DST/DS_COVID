...
def show_verification_backend():
    ...
    try:
        ...
        st.info(
            f"� Test de la fonction run_all_checks(): Toutes les vérifications passées: {result.get('all_checks_passed', 'N/A')}"
        )
        ...
        with st.expander("🔍 Diagnostic détaillé"):
            ...
            st.code(
                f"Dossier features: {Path(__file__).parent.parent.parent.parent / 'src' / 'features'}"
            )
...
def show_features_functions_analysis():
    ...
    if selected_file:
        ...
        if file_path:
            ...
            if functions_info:
                ...
                with col3:
                    documented = sum(
                        1 for f in functions_info if f["doc"] != "Pas de documentation"
                    )
    ...
    if st.checkbox("🌍 Afficher toutes les fonctions de tous les fichiers"):
        ...
        if all_functions:
            ...
            with col4:
                documented = sum(
                    1 for f in all_functions if f["doc"] != "Pas de documentation"
                )
